...
*   **방법 (README 및 코드 기반 추론)**:
    ...
    *   **관련 파일 추정 역할**:
        *   `src/cluster.py` 또는 `src/cluster.ipynb`: 실제 클러스터링 알고리즘을 구현하고 실행하는 메인 스크립트일 가능성이 높습니다. Word2Vec 벡터를 입력받아 응집형 계층적 클러스터링(예: `scikit-learn` 라이브러리 활용)을 수행하고, 각 단어의 클러스터 할당 결과를 생성하여 `clusters.py` (또는 유사 파일)에 저장합니다.
        *   `src/clusters.py` (및 `src/secondLevelClusters.py`): 클러스터링의 최종 결과를 **리스트의 리스트 (list of lists)** 형태로 저장하는 파일입니다. 바깥 리스트의 각 요소는 하나의 클러스터를 나타내는 내부 리스트이며, 각 내부 리스트는 해당 클러스터에 속한 단어(문자열)들로 구성됩니다. `qlearning.py`는 `from clusters import clusters` 구문을 통해 이 구조를 직접 로드하여 사용합니다. (`secondLevelClusters.py`의 존재는 클러스터링의 계층적 구조나 다단계 과정을 시사할 수 있습니다.)
...
*   **탐욕적 휴리스틱 (`qlearning.py`의 `choose_word_from_cluster` 함수)**:
    *   **역할**: 선택된 클러스터 내에서 이전 추측 중 최고/최저 유사도 단어를 기준으로 가장 유망한 단어를 선택합니다.
    *   **로직**: 이전 추측 최고점(`best_similarity`)이 임계값(20)보다 낮으면 최고/최저 유사도 단어와의 평균 유사도가 높은 단어를, 높으면 최고 유사도 단어와의 유사도에서 최저 유사도 단어와의 유사도를 뺀 값이 가장 큰 단어를 선택합니다. 이는 탐색(낮은 점수)과 활용(높은 점수) 간의 균형을 맞추려는 시도입니다.

*   **RNN 기반 휴리스틱 (`src/heuristicrnn.py`의 `RNN` 클래스, README 언급)**:
    *   **역할**: 이전 추측 단어들의 시퀀스를 입력받아 다음 추측 단어의 벡터나 유사도를 예측하는 RNN 모델을 사용합니다. 이를 통해 더 복잡한 패턴을 학습하여 단어를 선택할 수 있습니다.
    *   **구현**: `heuristicrnn.py`는 PyTorch(`torch.nn`)를 사용하여 RNN 모델(`class RNN`)을 정의합니다. 모델 구조 정의(`__init__`), 순전파(`forward`), 손실 계산(`compute_loss`), 모델 저장/로드 기능이 포함되어 있습니다.
    *   **결과**: README에 따르면, RNN 휴리스틱은 탐욕적 휴리스틱보다 성공률과 추측 속도 면에서 낮은 성능을 보였습니다.

*   **대체 휴리스틱 실험 (`Testing/heuristicAlternatives.py`)**: 
    *   이 파일에는 `choose_word_from_cluster_modified`와 같이 다른 방식의 휴리스틱 함수들이 포함되어 있을 수 있습니다. 이는 다양한 단어 선택 전략을 실험하고 비교하기 위한 코드일 가능성이 높습니다.
...
2.  **에피소드 반복**: 지정된 게임 수 (`num_games`)만큼 다음 과정을 반복합니다.
    ...
    b.  **스텝 반복**: 최대 추측 횟수(`max_guesses`) 또는 게임 종료 시까지 다음을 반복합니다.
        ...
        iii. **단어 선택 및 추측**: 선택된 클러스터 `a`에서 휴리스틱 함수(`choose_word_from_cluster` 또는 다른 휴리스틱)를 사용하여 단어 `w`를 선택하고 추측합니다 (`guess_word`).
...
4.  **스텝 반복**: 게임 종료 시까지 다음을 반복합니다.
    ...
    c.  **단어 선택 및 추측**: 선택된 최적 클러스터 `a`에서 휴리스틱 함수(`choose_word_from_cluster` 또는 사용하기로 결정된 다른 휴리스틱)를 사용하여 단어 `w`를 선택하고 추측합니다 (`guess_word`).
...
1.  **(사전 준비)** 단어 클러스터링 수행 및 결과 저장 (`cluster.py`, `clusters.py`).
2.  **(학습 단계)** 다수의 게임 플레이를 통해 Q-테이블 학습 (3.5. 학습 과정 상세 참조).
3.  **(추론 단계)** 학습된 Q-테이블과 낮은 엡실론 값을 사용하여 실제 게임 플레이 (3.6. 추론 과정 상세 참조).
...
*   **`src/answers.py`**: Semantle 게임의 가능한 정답 단어 목록(`secretWords`)을 파이썬 리스트 형태로 저장하고 있습니다. `qlearning.py` 등에서 게임 환경을 설정할 때 이 파일을 임포트하여 사용합니다.
*   **`Testing/` 디렉토리**: 다양한 테스트 및 실험 코드를 포함합니다.
    *   `heuristicAlternatives.py`: `choose_word_from_cluster_modified`와 같이, `qlearning.py`의 기본 탐욕적 휴리스틱 외에 다른 단어 선택 전략을 구현하고 테스트하기 위한 함수들을 포함할 수 있습니다.
    *   `runSingle.py`: `run_single_game` 함수를 통해, 학습된 Q-에이전트나 특정 설정을 이용하여 단일 Semantle 게임을 실행하고 그 결과를 관찰/평가하는 기능을 제공합니다.
